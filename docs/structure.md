...
## 3. 搜索索引（**待评估**，2026-09-08 调研后搁置）

**现状**：站内没有任何搜索。`hugo.toml` 没配 `outputs`，构建产物里 `find public -name "*.json"` 只有字体的 `reporter.json`。

**为什么要做**：文章一多，归档页靠翻页找文章就不好用了。

**为什么搁置**：调研发现"构建期出 JSON + 前端子串匹配"这条最省事的路**对中文是最弱的**——没有分词、没有排序、没有相关度，搜「排版设计」不会命中「排版与设计」。而中文分词恰恰是**自己造不出来的那块**。用户决定先不做。
...
**若要重开，推荐 Pagefind**，理由只有一条：中文分词是自己造不出来的那块。它的索引从**构建后的 HTML** 抓，所以目录式标记、`data-pagefind-ignore` 排除项都能精确控制；`<html lang="zh-cn">` 正好命中它多语言机制的 `zh-` 前缀。
...
2. **UI 是别人的**：能靠 CSS 变量调成宣纸色，但等于在别人的设计上刷漆，不如主题现有面板那么原生。要 100% 原生 UI 就只剩 FlexSearch 那条自写分词的路。
