...
- `layouts/` 是 0.146+ 顶层模板结构；`_markup/render-image.html`（page bundle 图片 → WebP/srcset，图片按约定收在文章目录的 `assets/` 下，见 `docs/writing/index.md`）和 `_markup/render-heading.html`（标题毛笔圈点 + 锚点）是渲染钩子
...
- **折叠块的内容体必须包一层 `.case-body`**（`shortcodes/details.html` 输出）：「抽出的宣纸」那条面板规则 `details[open] > :not(summary)` 的特异性是 `(0,1,2)`，**压得过 `.post-content ul` 的 `(0,1,1)`**。内容体直接当 `<details>` 的子元素时，面板的 `padding: 14px 18px` 会把列表自己的 `padding-left: 1.7em` 顶掉，`li::before` 挂在 `left:-1.02em` 的朱砂墨点随即贴到纸面左缘、探出框外（2026-09-11 修）。凡是写 `details > :not(summary)` 这类「一整个面板」的规则，都要先想到它会盖掉内容元素自己的 margin / padding；手写的 `<details>` 没有容器，走的就是老那条路径，行为不变
...
- **主题的列表圆点不是 `list-style`，是 `.post-content ul > li::before`**：一枚 5px 朱砂墨点 + 印泥晕，挂在 `left: -1.02em`（正文左缘之外）、`top: .68em`。两个后果：① 想在某处去掉圆点，**光写 `list-style: none` 是去不掉的**，必须显式 `content: none`；② 那两个值是按正文的字号与行高推出来的，**凡是在非正文语境里复用 `<ul>`（小字、注脚、更大的行距），墨点就会歪在文字左上方并探出左缘**——版权页踩过，当时误判成"标题记号的 27px 偏移"，绕了两轮才找到真凶
