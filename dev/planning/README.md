...
与 `../CLAUDE.md` 的分工（本文件是 `dev/planning/` 的封面）：

- `../CLAUDE.md` —— **已定的实现约定**与踩过的坑（做了的）
- `dev/planning/` —— **待办清单 + 理由**（没做的）

档位：**P0 已修** · **P1 已实施** · **P2 结构性**（改动大，看需求）。

> **状态（2026-09-08）**：P1 三篇（`01-seo-and-distribution.md` / `02-content-elements.md` / `03-rendering-pipeline.md`）已全部实施完毕，实现约定与踩到的坑已并入 `../CLAUDE.md`。这三篇已归档至 `dev/archive/` 作**决策记录**（为什么当初要做、当时怎么想），不再是待办。P2 的**菜单系统**与 **i18n 补齐**也已完成；**搜索索引**已由「能用」子串版升级为 **Pagefind**（2026-09-09，见 `structure.md` §3），「其他」一篇仍是待办。

---

## 速览

### P1 · 分发与 SEO（已实施，决策记录已归档 → [`../archive/01-seo-and-distribution.md`](../archive/01-seo-and-distribution.md)）
...
### P1 · 内容元素（已实施，决策记录已归档 → [`../archive/02-content-elements.md`](../archive/02-content-elements.md)）
...
### P1 · 渲染管线（已实施，决策记录已归档 → [`../archive/03-rendering-pipeline.md`](../archive/03-rendering-pipeline.md)）
...
**已完成**：菜单系统 ✅ · i18n 补齐 ✅ · 搜索索引 ✅（Pagefind，2026-09-09 升级）
...
### 待优化清单 —— [`optimizations.md`](optimizations.md)

把散在讨论里的待办沉淀成文（对照索引 + 站点发布前清理 + 内容/页面候选 + "倾向不做"备忘）。
...
4. 做完了：约定与坑并入 `../CLAUDE.md`；若整篇文档已完成，连同决策记录移入 `../archive/`（规矩：CLAUDE 记约定，archive 收已完成）
