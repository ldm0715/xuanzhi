...
> **更正（2026-09-10）**：上面第 3 条的改写**仍然不准确**。主题并非「零网络」——首页诗笺会向诗泉 API 发请求，而且**构建期与运行期各一次**（`home.html` 的 `resources.GetRemote` + `site.js:1015` 的 `fetch`），它才是唯一的对外请求。历次措辞从「零第三方运行时依赖」改成「零第三方网络依赖」，两次都是想当然，都没去核对 `site.js` 到底连了什么。准确表述见根 `CLAUDE.md` 开头。
